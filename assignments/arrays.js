...
let lastCar = 50;
console.log(`Last Car is number`,lastCar , `on the list and is a` , inventory[49]["car_make"] , inventory[49]["car_model"]);
...
let oldCars = [];
for (let item = 0; item < carYears.length; item++){
    if(carYears[item] < 2000){
        oldCars.push(carYears[item]);
    }
    else {
...
let BMWAndAudi = [];
for (let item = 0; item < inventory.length; item++){
    if (inventory[item].car_make == "BMW") {
        BMWAndAudi.push(inventory[item]);
    } else if (inventory[item].car_make == "Audi") {
        BMWAndAudi.push(inventory[item]);
    }
}
